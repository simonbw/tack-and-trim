...
pub fn build_mesh_data_from_tracks(
    ...
    let ensure_vertices = |seg: &WavefrontSegment, vertices: &mut Vec<f32>, vertex_offset: &mut usize, base_by_segment: &mut HashMap<*const WavefrontSegment, usize>| -> usize {
        ...
        for pi in 0..seg.len() {
            ...
            vertices[*vertex_offset] = seg.turbulence[pi] as f32; *vertex_offset += 1;
            vertices[*vertex_offset] = phase_offset as f32; *vertex_offset += 1;
            vertices[*vertex_offset] = seg.blend[pi] as f32; *vertex_offset += 1;
        }
        base
    };
    ...
    for track in tracks {
        for si in 0..track.snapshots.len().saturating_sub(1) {
            ...
            let prev_base = ensure_vertices(prev_seg, &mut vertices, &mut vertex_offset, &mut base_by_segment);
            let next_base = ensure_vertices(next_seg, &mut vertices, &mut vertex_offset, &mut base_by_segment);
            index_offset = triangulate_segment_pair(
                prev_seg, next_seg, prev_base, next_base, &mut indices, index_offset,
            );
        }
    }
    ...
    for parent in tracks {
        let last_snap = match parent.snapshots.last() { Some(s) => s, None => continue };
        for &child_id in &parent.child_track_ids {
            let child_track = match track_by_id.get(&child_id) { Some(t) => t, None => continue };
            let first_snap = match child_track.snapshots.first() { Some(s) => s, None => continue };
            let parent_base = ensure_vertices(&last_snap.segment, &mut vertices, &mut vertex_offset, &mut base_by_segment);
            let child_base = ensure_vertices(&first_snap.segment, &mut vertices, &mut vertex_offset, &mut base_by_segment);
            index_offset = triangulate_segment_pair(
                &last_snap.segment, &first_snap.segment,
                parent_base, child_base, &mut indices, index_offset,
            );
        }
    }
...
fn count_mesh_topology(tracks: &[SegmentTrack]) -> MeshTopology {
    ...
    let mut unique_ptrs: std::collections::HashSet<*const WavefrontSegment> = std::collections::HashSet::new();
    ...
    for track in tracks {
        ...
        for si in 0..track.snapshots.len().saturating_sub(1) {
            triangle_count += count_triangles_between(
                &track.snapshots[si].segment, &track.snapshots[si + 1].segment,
            );
        }
    }

    for parent in tracks {
        let last = match parent.snapshots.last() { Some(s) => s, None => continue };
        for &child_id in &parent.child_track_ids {
            let child = match track_by_id.get(&child_id) { Some(t) => t, None => continue };
            let first = match child.snapshots.first() { Some(s) => s, None => continue };
...
fn triangulate_segment_pair(
    ...
    while i < p_end || j < n_end {
        if i >= p_end {
            indices[offset] = (prev_base + i) as u32; offset += 1;
            indices[offset] = (next_base + j) as u32; offset += 1;
            indices[offset] = (next_base + j + 1) as u32; offset += 1;
            j += 1;
        } else if j >= n_end {
            indices[offset] = (prev_base + i) as u32; offset += 1;
            indices[offset] = (prev_base + i + 1) as u32; offset += 1;
            indices[offset] = (next_base + j) as u32; offset += 1;
            i += 1;
        } else {
            let score_a = score_triangle(
                prev.x[i], prev.y[i], prev.x[i + 1], prev.y[i + 1], next.x[j], next.y[j],
            );
            let score_b = score_triangle(
                prev.x[i], prev.y[i], next.x[j], next.y[j], next.x[j + 1], next.y[j + 1],
            );
            if score_a < score_b {
                indices[offset] = (prev_base + i) as u32; offset += 1;
                indices[offset] = (prev_base + i + 1) as u32; offset += 1;
...
fn score_triangle(ax: f64, ay: f64, bx: f64, by: f64, cx: f64, cy: f64) -> f64 {
    ...
    let d2x = cx - bx; let d2y = cy - by;
    let d3x = ax - cx; let d3y = ay - cy;
...
fn clip_to_range(seg: &WavefrontSegment, min_t: f64, max_t: f64) -> (usize, usize) {
    ...
    let len = t.len();
    let mut start = 0;
    while start < len && t[start] < min_t { start += 1; }
    let mut end = len - 1;
...
fn compute_coverage_quad(bounds: &WaveBounds, wave_dx: f64, wave_dy: f64) -> CoverageQuad {
    ...
    let to_world = |proj: f64, perp: f64| -> (f64, f64) {
        (proj * wave_dx + perp * perp_dx, proj * wave_dy + perp * perp_dy)
    };
