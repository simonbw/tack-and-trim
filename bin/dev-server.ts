...
const PARCEL_PORT = 1235;
const PROXY_PORT = 1234;

// Start Parcel on a different port
const parcel = spawn("npx", ["parcel", "--no-hmr", "--port", String(PARCEL_PORT), "src/index.html"], {
  stdio: "inherit",
  shell: true,
});
...
server.listen(PROXY_PORT, () => {
  ...
  console.log(`   (proxying Parcel on port ${PARCEL_PORT} with COOP/COEP headers)\n`);
});
