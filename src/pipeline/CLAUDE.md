...
| Module | Purpose |
|--------|---------|
| `region.ts` | Loads `region.json`, resolves `--region` CLI flag, path helpers |
| `geo-utils.ts` | Lat/lon ↔ feet projection, bbox math, CUDEM tile filename parsing |
| `marching-squares.ts` | `ScalarGrid`, `MarchSegments` types, `buildClosedRings()` ring tracer, `BlockIndex` for fast level-skip |
| `grid-cache.ts` | Lists local tiles by bbox, reads GeoTIFF metadata |
| `simplify.ts` | Ramer-Douglas-Peucker for polylines and closed rings, `signedArea`, `ringPerimeter` |
| `segment-index.ts` | Spatial grid for fast segment intersection queries |
| `constrained-simplify.ts` | RDP simplification that refuses to collapse spans that would cross already-finalized contours |
| `worker-pool.ts` | `ContourWorkerPool` — distributes marching squares across worker threads using `SharedArrayBuffer` |
| `workers/contour-worker.ts` | Worker thread: computes block index, runs `marchCell()` with full 16-case lookup table + saddle disambiguation |
