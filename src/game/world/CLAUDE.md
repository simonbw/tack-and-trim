...
All terrain GPU data is packed into a single `array<u32>` buffer (`packedTerrainBuffer`) with a 4-element header `[verticesOffset, contoursOffset, childrenOffset, containmentGridOffset]` followed by the data sections. The containment grid section stores a precomputed 64x64 grid of 2-bit inside/outside/boundary flags per contour, enabling O(1) containment tests for ~95% of queries. Accessor functions in `shaders/terrain-packed.wgsl.ts` (`getTerrainVertex`, `getContourData`, `getTerrainChild`, `getContainmentCellFlag`) read from this buffer using `bitcast<f32>()` for float fields. The contour section uses mixed u32/f32 data -- writing integer fields through a Float32Array would produce wrong bit patterns, so packing uses `DataView` / `Uint32Array` with careful handling. All contours are normalized to CCW winding on load.
