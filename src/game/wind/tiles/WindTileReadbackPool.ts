/**
 * Pool of readback buffers for wind tiles.
 *
...
import {
  WindReadbackBuffer,
  WindReadbackViewport,
  WindVelocity,
} from "../webgpu/WindReadbackBuffer";
import type { WindTile } from "./WindTileTypes";

/**
 * Statistics for wind tile readback operations.
 */
export interface WindTileReadbackStats {
 ...
 * Pool of readback buffers for wind tiles.
 */
export class WindTileReadbackPool {
  private buffers: WindReadbackBuffer[] = [];
  private tileAssignments = new Map<number, WindTile>();
  private initialized = false;

  readonly stats: WindTileReadbackStats = {
    tileHits: 0,
    cpuFallbacks: 0,
  ...
  async init(): Promise<void> {
    ...
    for (let i = 0; i < this.poolSize; i++) {
      const buffer = new WindReadbackBuffer(this.tileResolution);
      await buffer.init();
   ...
   */
  assignBuffersToTiles(tiles: WindTile[]): void {
   ...
   */
  getBufferForTile(tile: WindTile): WindReadbackBuffer | null {
   ...
   * Get buffer by index.
   */
  getBuffer(index: number): WindReadbackBuffer | null {
   ...
   */
  getTileForBuffer(index: number): WindTile | undefined {
   ...
   * Returns wind velocity or null if the point is not in the tile's viewport.
   */
  sampleAtWorldPoint(
    tile: WindTile,
    worldX: number,
