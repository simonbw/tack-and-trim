/**
 * Manages the tile grid and scoring for physics water computation.
 *
 * Responsibilities:
...
import type { QueryForecast } from "../WaterQuerier";
import {
  DEFAULT_TILE_CONFIG,
  getTileBounds,
  Tile,
...
/**
 * Manages the tile grid and scoring for physics computation.
 */
export class TileManager {
  ...
  constructor(config: Partial<TileGridConfig> = {}) {
   ...
   */
  selectTilesToCompute(_currentTime: number): Tile[] {
   ...
   */
  private getOrCreateTile(gridX: number, gridY: number): Tile {
    const id = toTileId(gridX, gridY);
   ...
   */
  findTileForPoint(worldX: number, worldY: number): Tile | null {
    const [gridX, gridY] = worldToGrid(worldX, worldY, this.config.tileSize);
    const id = toTileId(gridX, gridY);
